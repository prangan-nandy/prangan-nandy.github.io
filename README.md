...
`Prangan\_Nandy\_CV.pdf` — the PDF opened by every CV button
`Prangan\_Nandy\_Resume.pdf` — the one-page resume
...
Click Add file.
Click Upload files.
Drag these files from the extracted folder into the upload area:
`index.html`
`style.css`
`script.js`
`profile-photo.jpg`
...
`favicon.svg`
`Prangan\_Nandy\_CV.pdf`
`Prangan\_Nandy\_Resume.pdf`
GitHub may warn that some files already exist. That is expected: the upload will replace their contents after the commit.
In the commit-message box, enter: `Update portfolio design and content`
...
Choose the photo on your computer.
Crop it vertically if needed.
Export it as a JPG.
Rename the file exactly: `profile-photo.jpg`
Open the GitHub repository.
Click Add file → Upload files.
Upload the new `profile-photo.jpg`.
Commit with: `Replace profile photo`
...
Export the updated CV as PDF.
Rename it exactly: `Prangan\_Nandy\_CV.pdf`
Upload it to the repository root through Add file → Upload files.
Replace the existing file.
Commit with: `Update academic CV`
...
Open `index.html` in the GitHub repository.
Click the pencil icon.
Use `Ctrl + F` or `Command + F` to find the exact sentence, project title, employer, or section heading.
Edit only the words between the HTML tags.
Commit the change.
Example:
```html
...
The public contact section currently shows the work and personal email addresses suppl
